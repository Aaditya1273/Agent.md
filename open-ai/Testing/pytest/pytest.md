...
- [ ] Every skip has a reason; every `xfail` references an issue
- [ ] Fakes target interfaces we own; vendor SDKs are never mocked directly
- [ ] `monkeypatch` and `AsyncMock` used; real network blocked
- [ ] `asyncio_mode = "auto"`; no real sleeps in async tests
