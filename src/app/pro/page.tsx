...
export default function ProPage() {
  ...
  return (
    <div className="min-h-screen font-sans" style={{ background: "var(--background)", color: "var(--foreground)" }}>
      {/* Nav */}
      <nav className="sticky top-0 z-50 w-full" style={{ background: "var(--card)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(0,0,0,0.06)" }}>
        <div className="max-w-screen-xl mx-auto px-6 h-14 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2">
            <div className="size-6 rounded-md flex items-center justify-center" style={{ background: "var(--foreground)" }}>
              <div className="size-2 rotate-45" style={{ background: "var(--background)" }} />
            </div>
            <span className="font-mono font-semibold tracking-tighter text-[15px]">AGENT.MD</span>
          </Link>
          <div className="flex items-center gap-2">
            <Link href="/" className="px-3 py-1.5 text-xs font-medium rounded-lg transition-colors" style={{ color: "var(--muted-foreground)" }}>
              Browse presets
            </Link>
            <button className="px-3 py-1.5 text-xs font-semibold rounded-lg" style={{ background: "var(--foreground)", color: "var(--background)" }}>
              Sign in
            </button>
          </div>
        </div>
      ...
      <div className="flex justify-center mb-10">
        <div className="flex items-center p-1 rounded-xl" style={{ background: "var(--card)", boxShadow: "0 1px 3px rgba(0,0,0,0.07), 0 0 0 1px rgba(0,0,0,0.06)" }}>
          {PLANS.map((p) => (
            <button
              key={p.id}
      ...
      <section className="max-w-screen-lg mx-auto px-6 pb-20">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 max-w-3xl mx-auto">
          ...
          <div className="rounded-2xl p-7 flex flex-col"
            ...
            <Link href="/" className="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl text-sm font-semibold transition-colors"
          ...
          <div className="rounded-2xl p-7 flex flex-col relative overflow-hidden"
            ...
            <div className="relative mb-6">
              ...
              {plan.total && (
                <p className="text-xs mt-1 opacity-50">{plan.total}</p>
              )}
              <p className="text-sm mt-2 opacity-70">
                Full access to every premium pack, download rights, and API access.
              </p>
            </div>
            ...
            <button className="relative flex items-center justify-center gap-2 w-full py-3 rounded-xl text-sm font-bold transition-all hover:opacity-90"
              style={{ background: "var(--primary)", color: "var(--primary-foreground)" }}>
