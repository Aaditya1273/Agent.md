...
- [ ] Long handlers check a shutdown flag at checkpoints
- [ ] Handlers are idempotent so a `SIGKILL` is survivable
- [ ] Workloads are separated into pools by shape and criticality
- [ ] Untrusted work runs in an isolated, network-restricted pool
- [ ] Autoscaling uses backlog age or depth, not CPU
- [ ] Maximum replicas are capped at downstream capacity
- [ ] Liveness detects a stalled handler loop and checks no dependencies
