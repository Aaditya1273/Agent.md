...
- [ ] Scheduled jobs run once across all replicas
- [ ] Schedules are in UTC, jittered, and non-overlapping
- [ ] Absence of a successful scheduled run raises an alert
- [ ] Worker concurrency is bounded by the narrowest downstream resource
- [ ] Memory use is bounded; large datasets are streamed
- [ ] Fast and slow workloads run on separate pools
- [ ] Job type, outcome, duration and attempt are emitted as metrics
- [ ] Trace context propagates from the enqueuing request
