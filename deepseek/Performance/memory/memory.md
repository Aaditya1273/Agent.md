...
- [ ] Database result sets are paginated or cursored
- [ ] Exports and reports stream rather than building in memory
- [ ] Stream backpressure is respected
- [ ] Leaks are diagnosed by snapshot comparison and retainer paths
- [ ] Growth and leaks are distinguished by recovery after load stops
- [ ] Container-level memory metrics are monitored, not only heap size
