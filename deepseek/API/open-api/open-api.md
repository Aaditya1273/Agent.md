...
- [ ] `securitySchemes` are declared and applied per operation
- [ ] Constraints (`pattern`, `minimum`, `maxLength`) are expressed in the schema
- [ ] CI lints the spec against house rules
- [ ] CI fails on a breaking diff against the merge base
- [ ] The running service is tested against the spec
