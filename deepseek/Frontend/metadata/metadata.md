...
- [ ] Every route generates its own title and description
- [ ] Titles are within ~60 characters, most specific part first
- [ ] Descriptions are within ~155 characters, human-readable, markup-free
- [ ] Open Graph and Twitter tags are present with absolute image URLs
- [ ] Social images are 1200×630 and cached immutably
- [ ] `noindex` is applied deliberately and never leaks to production
- [ ] A smoke test asserts production pages are indexable
- [ ] Canonical URLs are set per page
- [ ] Icons, manifest and `theme-color` are configured
