...
- [ ] `Vary: Origin` is set whenever the allowed origin varies
- [ ] `Allow-Credentials` is enabled only where cookies are genuinely required
- [ ] `Allow-Methods` and `Allow-Headers` list only what is accepted
- [ ] `Max-Age` is modest
