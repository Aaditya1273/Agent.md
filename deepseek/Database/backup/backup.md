...
- [ ] Backups are stored in a separate account with separate credentials
- [ ] At least one copy is immutable for its retention window
- [ ] Retention tiers cover operational, compliance and legal needs
- [ ] Backups are encrypted, with keys held outside the backup system
- [ ] A restore drill runs at least monthly and its duration is recorded
